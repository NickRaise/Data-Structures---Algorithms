// Problem: Find Pivot Index
// Given an array of integers nums, calculate the pivot index of this array.

// The pivot index is the index where the sum of all the numbers strictly
// to the left of the index is equal to the sum of all the numbers strictly to the index's right.

// If the index is on the left edge of the array, then the left sum is 0 because there are no elements to the left. 
// This also applies to the right edge of the array.

// Return the leftmost pivot index. If no such index exists, return -1.

// Example 1:
// Input: nums = [1,7,3,6,5,6]
// Output: 3
// Explanation:
// The pivot index is 3.
// Left sum = nums[0] + nums[1] + nums[2] = 1 + 7 + 3 = 11
// Right sum = nums[4] + nums[5] = 5 + 6 = 11




#include <iostream>
#include <vector>
#include <numeric>

using namespace std;

int pivotIndex(vector<int>& nums) {
    int n = nums.size();
    int lsum = 0;  // Initialize the sum of elements to the left of the pivot
    int rsum = accumulate(nums.begin(), nums.end(), 0);  // Calculate the total sum of elements
        
    int i = 0;
    while (i < n) {
        rsum -= nums[i];  // Subtract the current element from the right sum
        if (lsum == rsum) {
            return i;  // If left and right sums are equal, return the current index as pivot
        }
        lsum += nums[i];  // Add the current element to the left sum
        i++;
    }
    return -1;  // No pivot index found
}

int main() {
    int n;
    cin >> n;
    
    vector<int> nums(n);
    for (int i = 0; i < n; ++i) {
        cin >> nums[i];
    }

    int pivotIdx = pivotIndex(nums);
    if (pivotIdx != -1) {
        cout << "Pivot index: " << pivotIdx << endl;
    } else {
        cout << "No pivot index found." << endl;
    }

    return 0;
}
